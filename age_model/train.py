...
experiment.set_name(str("ordinal_loss"))
...
def train_model(epochs=EPOCHS, batch_size=BATCH_SIZE, lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY, alpha=ALPHA, dropout=DROPOUT):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} | "
              ...
              f"\nTrain Acc: {train_acc:.2f}% | ±3: {train_acc_3:.1f}% | ±5: {train_acc_5:.1f}% | ±10: {train_acc_10:.1f}%) | "
              f"\nVal Loss: {val_loss_avg:.4f} | "
              f"\nVal Acc: {val_acc:.2f}% | ±3: {val_acc_3:.1f}% | ±5: {val_acc_5:.1f}% | ±10: {val_acc_10:.1f}%")

        if val_acc_3 > best_val_acc_3:
            best_val_acc_3 = val_acc_3
            patience_counter = 0
            checkpoint_path = os.path.join(CHECKPOINTS_PATH, "age_model_best.pth")
            torch.save({
                'epoch': epoch,
            ...
            print(f"Saved best model with val_acc_3: {val_acc_3:.2f}% (exact:{val_acc:.2f}% ±5:{val_acc_5:.1f}% ±10:{val_acc_10:.1f}%)")
            experiment.log_asset(checkpoint_path, file_name="age_model_best.pth")
        else:
            patience_counter += 1
    ...
    print(f"Training complete. Best validation accuracy (±3): {best_val_acc_3:.2f}%")
    experiment.log_parameter("best_val_acc", best_val_acc_3)
    experiment.end()
